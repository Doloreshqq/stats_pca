...
h2 <- read.csv("df9_2_hidden.csv")
h3 <- read.csv("df9_3_hidden.csv")
h4 <- read.csv("df9_4_hidden.csv")
h5 <- read.csv("df9_5_hidden.csv")
```
...
all_data <- data.frame(probability = c(predicted_values, h1$pred_prob, h2$pred_prob, h3$pred_prob, h4$pred_prob, h5$pred_prob),
                        model = c(rep("glm", length(predicted_values)),
                                  rep("hidden 1", length(h1$pred_prob)),
                                  rep("hidden 2", length(h2$pred_prob)),
                                  rep("hidden 3", length(h3$pred_prob)),
                                  rep("hidden 4", length(h4$pred_prob)),
                                  rep("hidden 5", length(h5$pred_prob))))
...
all_data1$model <- factor(all_data1$model, levels = c("glm", "epoch 2", "epoch 5", "epoch 10", "epoch 20", "epoch 50"))
...
```{r}
es <- c(coef(model_h1)[-1], coef(model_h2)[-1],coef(model_h3)[-1],coef(model_h4)[-1],coef(model_h5)[-1])
ers <- c(   summary(model_h1)$coef[-1, "Std. Error"],
...
# 创建数据框
daa1 <- data.frame(Model = rep(c("hidden 1", "hidden 2", "hidden 3", "hidden 4", "hidden 5"), each = length(es)/5),
                   Variable = c(1:350),
                   Estimate = es,
                   Error = ers)
...
ppp1 <- ggplot(daa1, aes(x = Variable, y = Estimate, fill = Model, color = Model)) +
  ...
  scale_fill_manual(values = c("hidden 1" = "lightpink1",
                               "hidden 2" = "lightsalmon1",
                               "hidden 3" = "tomato1",
                               "hidden 4" = "red2",
                               "hidden 5" = "brown3")) +
  scale_color_manual(values = c("hidden 1" = "lightpink",
                               "hidden 2" = "lightsalmon",
                               "hidden 3" = "tomato2",
                               "hidden 4" = "red3",
                               "hidden 5" = "brown4")) +
