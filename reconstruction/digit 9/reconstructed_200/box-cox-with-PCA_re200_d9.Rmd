...
```{r}
# Load the dataset
d1 <- read.csv("pca_data_re200_20_d9.csv") 
d2 <- read.csv("pca_data_re200_40_d9.csv") 
d3 <- read.csv("pca_data_re200_60_d9.csv") 
d4 <- read.csv("pca_data_re200_80_d9.csv") 
d5 <- read.csv("pca_data_re200_100_d9.csv") 
d6 <- read.csv("pca_data_re200_120_d9.csv") 
d7 <- read.csv("pca_data_re200_140_d9.csv") 
d8 <- read.csv("pca_data_re200_160_d9.csv") 
d9 <- read.csv("pca_data_re200_180_d9.csv") 
d10 <- read.csv("pca_data_re200_200_d9.csv") 
d11 <- read.csv("pca_data_re200_250_d9.csv") 
d12 <- read.csv("pca_data_re200_300_d9.csv")
d13 <- read.csv("pca_data_re200_350_d9.csv")
d14 <- read.csv("pca_data_re200_400_d9.csv")
...
```{r}
group = c(20,40,60,80,100,120,140,160,180,200,250,300,350,400)
...
plot <- ggplot(dp, aes(x=pca)) +
  ...
  labs(title="R2 and Adjusted R2 vs PCA for digit 9 (reconstruction)",
       x="PCA",
       y="R2 / Adjusted R2") +
  theme_minimal() +
...
```{r}
pca <- c(20,40,60,80,100,120,140,160,180,200,250,300,350,400)
ad_r2 <- c(0.279,0.3603,0.3762,0.3709,0.3842,0.3843,0.3781,0.3728,0.3652,0.352,0.3149,0.3159,0.3669,0.3011)

dp <- data.frame(pca,ad_r2, ar2)

plot <- ggplot(dp, aes(x=pca)) +
  geom_line(aes(y=ad_r2, color="Adjusted R2 (origin)"), size=1.2) +
  geom_line(aes(y=ar2, color="Adjusted R2 (reconstruction)"), size=1.2) +
  labs(title="Adjusted R2 vs PCA for digit 9",
       x="PCA",
       y="Adjusted R2 (origin) / Adjusted R2 (reconstruction)") +
  theme_minimal() +
  scale_color_manual(values=c("Adjusted R2 (origin)"="blue", "Adjusted R2 (reconstruction)"="red")) +
  theme(legend.position="top")
