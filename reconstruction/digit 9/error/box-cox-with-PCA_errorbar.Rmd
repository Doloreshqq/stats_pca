...
```{r}
# Load the dataset
d1 <- read.csv("pca_data_re100_100_d9.csv") 
d2 <- read.csv("pca_data_re150_100_d9.csv") 
d3 <- read.csv("pca_data_re200_100_d9.csv") 
d4 <- read.csv("pca_data_100_d9.csv") 
...
nd_list <- list(nd1, nd2, nd3, nd4)
```

```{r}
vars_set1 <- paste("f_", 1:100, sep = "")
rrr = paste(vars_set1, collapse = "+")
formula <- as.formula(paste("pred_prob ~", rrr))
model_or <- lm(formula, data = nd4)
model_100 <- lm(formula, data = nd1)
model_150 <- lm(formula, data = nd2)
model_200 <- lm(formula, data = nd3)

```

```{r}
# 假设你有四个模型：model_or, model_100, model_150, model_200

# 提取估计值（不包括截距）和误差
estimates <- c(coef(model_or)[-1], coef(model_100)[-1], coef(model_150)[-1], coef(model_200)[-1])
errors <- c(summary(model_or)$coef[-1, "Std. Error"], 
            summary(model_100)$coef[-1, "Std. Error"],
            summary(model_150)$coef[-1, "Std. Error"],
            summary(model_200)$coef[-1, "Std. Error"])

# 创建数据框
daa <- data.frame(Model = rep(c("origin", "100", "150", "200"), each = length(estimates)/4),
                   Variable = c(1:100),
                   Estimate = estimates,
                   Error = errors)
...
ppp <- ggplot(daa, aes(x = Variable, y = Estimate, fill = Model, color = Model)) +
  geom_point(data = subset(daa, Model == "origin"), size = 2, position = position_dodge(width = 0.8)) +
  geom_point(data = subset(daa, Model != "origin"), size = 1, position = position_dodge(width = 0.8)) +

  # 用 scale_color_manual 自定义颜色
  scale_color_manual(values = c("origin" = "black", "100" = "burlywood4", "150" = "skyblue3", "200" = "pink3")) +
  scale_fill_manual(values = c("origin" = "gray","100" = "burlywood3", "150" = "skyblue2", "200" = "pink2")) +

  # 为 model_or 的 error bar 上下两端加粗
  geom_ribbon(data = subset(daa, Model == "origin"), 
              aes(ymin = Estimate - Error, ymax = Estimate + Error, fill = Model), 
              alpha = 0.3, color = NA) +  # 移除边框，设置 color = NA
  geom_errorbar(data = subset(daa, Model != "origin"), 
